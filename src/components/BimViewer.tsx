      ...
      (results: any[]) => {
        ...
        results.forEach(({ dbId, properties }: any) => {
          properties.forEach(({ displayValue }: any) => {
            if (displayValue) {
              nameToDbId.current[String(displayValue).trim()] = dbId;
            }
          });
        });
          ...
          {detailItem ? (
            <div className="space-y-3">
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">{"קומה"}</span><span className="font-inter">{detailItem.floor}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">{"יחידה"}</span><span className="font-inter">{detailItem.unit}</span></div>
